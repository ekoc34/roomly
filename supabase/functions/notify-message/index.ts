...
//   2. Unique index + debounce make this idempotent: same IDs → at most 1 email.
//   3. No sensitive data is returned in responses, only status strings.
...
Deno.serve(async (req) => {
  ...
  // ── Resolve conversation → recipient ──────────────────────────
  const { data: conv, error: convErr } = await admin
    .from("conversations")
    .select("id, tenant_id, landlord_id, listings(title)")
  ...
  // ── Debounce: max 1 email per conversation per 10 minutes ─────
