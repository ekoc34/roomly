...
export function RegisterPage() {
  ...
  const signInWithOAuth = async (provider: "google" | "facebook") => {
    ...
    if (err) {
      setOauthError(provider === "google" ? "Kon niet inloggen met Google." : "Kon niet inloggen met Facebook.");
          ...
          {checkEmail ? (
            <div className="flex flex-col items-center gap-5 py-4 text-center">
              ...
              <div>
                ...
                <h1 className="mt-3 text-xl font-bold text-stone-900">{t("auth.checkInboxTitle")}</h1>
                <p className="mt-3 text-sm leading-relaxed text-stone-500">
                  {t("auth.checkInboxDesc").replace("{email}", "")}
                  <span className="font-semibold text-stone-700">{checkEmail}</span>.
                </p>
                <p className="mt-2 text-xs text-stone-400">{t("auth.noEmailReceived")} Check ook je spammap.</p>
              </div>
              <div className="w-full space-y-2 pt-2">
                <p className="text-xs text-stone-400">
                  ...
                  <button
                    type="button"
                    onClick={() => { setCheckEmail(null); setError(null); }}
            ...
            <>
              <div className="text-center">
                <p className="text-3xl font-black text-rose-600">Welkthuis</p>
                <h1 className="mt-2 text-xl font-bold text-stone-900">{t("auth.registerTitle")}</h1>
                <p className="mt-1 text-sm text-stone-500">{t("auth.registerSubtitle")}</p>
              </div>
              ...
              <div className="mt-6 space-y-3">
                ...
                >
                  ...
                  {oauthLoading === "google" ? t("common.loading") : t("auth.continueGoogle")}
                </button>
                ...
                >
                  ...
                  {oauthLoading === "facebook" ? t("common.loading") : t("auth.continueFacebook")}
                </button>
              </div>

              <div className="my-5 flex items-center gap-3">
                <div className="h-px flex-1 bg-stone-200" />
                <span className="text-xs text-stone-400">{t("auth.orDivider")}</span>
                <div className="h-px flex-1 bg-stone-200" />
              </div>

              <form onSubmit={onSubmit} className="space-y-4" data-testid="register-form">
                <div>
                  <label htmlFor="reg-name" className="text-xs font-medium text-stone-700">{t("auth.nameLabel")}</label>
                  <input id="reg-name" name="name" type="text" required data-testid="register-name" className="mt-1.5 w-full rounded-xl border border-stone-200 bg-white px-4 py-2.5 text-sm text-stone-900 placeholder:text-stone-400 focus:border-rose-300 focus:outline-none focus:ring-2 focus:ring-rose-200" placeholder={t("auth.namePlaceholder")} />
                </div>
                <div>
                  <label htmlFor="reg-email" className="text-xs font-medium text-stone-700">{t("auth.emailLabel")}</label>
                  <input id="reg-email" name="email" type="email" required autoComplete="email" data-testid="register-email" className="mt-1.5 w-full rounded-xl border border-stone-200 bg-white px-4 py-2.5 text-sm text-stone-900 placeholder:text-stone-400 focus:border-rose-300 focus:outline-none focus:ring-2 focus:ring-rose-200" placeholder={t("auth.emailPlaceholder")} />
                </div>
                <div>
                  <label htmlFor="reg-password" className="text-xs font-medium text-stone-700">{t("auth.passwordLabel")}</label>
                  <div className="mt-1.5">
                    <PasswordInput id="reg-password" name="password" required minLength={8} data-testid="register-password" placeholder={t("auth.passwordHint")} autoComplete="new-password" />
                  </div>
                </div>
                <div>
                  <label htmlFor="reg-confirm" className="text-xs font-medium text-stone-700">{t("auth.confirmPasswordLabel")}</label>
                  <div className="mt-1.5">
                    <PasswordInput id="reg-confirm" name="confirm" required minLength={8} data-testid="register-confirm" placeholder={t("auth.confirmPasswordPlaceholder")} autoComplete="new-password" />
                  </div>
                </div>
              ...
              <p className="mt-5 text-center text-sm text-stone-500">
                {t("auth.alreadyAccount")}{" "}
                <Link href={nextParam !== "/dashboard" ? `/inloggen?next=${encodeURIComponent(nextParam)}` : "/inloggen"} data-testid="register-login-link" className="font-medium text-rose-600 hover:underline">{t("auth.loginLink")}</Link>
              </p>
            </>
          )}
