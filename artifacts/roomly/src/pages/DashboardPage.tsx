  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      const [
        ...
        supabase!.from("listings").select("id, title, price, location, images, boosted_at, created_at").order("created_at", { ascending: false }).limit(4),
      ]);
      ...
      setRecommendedListings((recListings as RecommendedListing[] | null) ?? []);
          ...
          <div className="mt-6 space-y-8">
            ...
            {!loading && profile && (
              <div className={`rounded-2xl border p-5 shadow-sm ${profilePct === 100 ? "border-emerald-200 bg-emerald-50/60" : "border-stone-200/80 bg-white"}`}>
                ...
                {profilePct < 100 && missingProfileFields.length > 0 && (
                  <div className="mt-4 space-y-2">
                    {missingProfileFields.map((f) => (
                      <div key={f.key} className="flex items-center justify-between gap-3 text-sm text-stone-600">
                        <span className="flex items-center gap-2 text-xs text-stone-500">
                          <span className="h-1.5 w-1.5 rounded-full bg-stone-300" />
                          {f.missing}
                        </span>
                        <Link href="/profiel" className="shrink-0 text-xs font-semibold text-rose-600 hover:underline">Toevoegen</Link>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {profilePct < 100 && (
                  <Link href="/profiel" className="mt-4 inline-flex items-center gap-1.5 rounded-xl bg-rose-500 px-4 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-rose-600 active:scale-95">
                    Profiel verbeteren →
                  </Link>
                )}
              </div>
                  ...
                  {myApplications.map((app) => {
                    ...
                    return (
                      <div key={app.id} className="relative flex flex-col gap-3 rounded-2xl border border-stone-200/80 bg-white p-5 pr-10 shadow-sm sm:flex-row sm:items-center">
                        ...
                        {app.status === "accepted" && conv && (
                          <Link href={`/berichten/${conv.id}`} className="shrink-0 inline-flex items-center gap-1.5 rounded-xl bg-emerald-500 px-3 py-2 text-xs font-semibold text-white transition hover:bg-emerald-600">
                            <MessageSquare className="h-3.5 w-3.5" /> Ga naar gesprek
                          </Link>
                        )}
                        {app.listings?.id && (
                          <Link href={`/kamers/${app.listings.id}`} className="shrink-0 inline-flex items-center gap-1 rounded-xl border border-stone-200 px-3 py-2 text-xs font-medium text-stone-600 transition hover:bg-stone-50">
                            Bekijk woning →
                          </Link>
                        )}
                      </div>
            ...
            {/* 4. Aanbevolen voor jou */}
            <div>
              <div className="mb-4 flex items-center gap-2">
                <h2 className="text-lg font-bold text-stone-900">Aanbevolen voor jou</h2>
                <span className="rounded-full bg-rose-100 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-rose-600">Nieuw</span>
              </div>
              {loading ? (
          ...
          <div className="mt-8 rounded-3xl border border-amber-200 bg-amber-50/60 p-6 shadow-sm">
            <div className="mb-4 flex items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-xs text-stone-500">Test de iDEAL betalingen voor Boost Credits. Alleen zichtbaar voor admins.</p>
              </div>
            </div>
