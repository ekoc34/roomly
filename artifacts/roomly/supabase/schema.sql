...
--    The previous constraint silently rejected every saved-search notification.
ALTER TABLE public.notifications
  DROP CONSTRAINT IF EXISTS notifications_type_check;
ALTER TABLE public.notifications
  ADD CONSTRAINT notifications_type_check
...
-- 2. Replace the open INSERT policy with one that restricts users to
--    inserting notifications addressed only to themselves.
--    The server-side RPC function (SECURITY DEFINER) bypasses RLS to
--    insert for other users — the browser never can.
...
--    Mirrors the filter logic previously in the browser (listingMatchesFilters).
--    The caller must own the listing; all cross-user inserts happen here only.
...
BEGIN
  ...
  LOOP
    -- Mirror listingMatchesFilters: skip if any active filter does not match
    CONTINUE WHEN
      -- q: full-text search across title, description, location
      (
      ...
      -- max price (10000 = no upper bound)
      (
