...
export default async function KamersPage({
  ...
  if (getSupabaseConfig() && listings.length > 0) {
    try {
      ...
      if (user) {
        ...
        favoriteIds = (data ?? []).map((r: { listing_id: string }) => r.listing_id);
      }
    } catch {
      favoriteIds = [];
    }
    ...
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="max-w-2xl">
        ...
        <p className="mt-2 text-sm text-stone-500 sm:text-base">
          Filter op prijs, stadsdeel en type. Kamers, appartementen en korte
          verblijven — voor iedereen die een thuis zoekt.
        </p>
      </div>
      ...
      <div className="mt-10">
        {listings.length > 0 && (
          <p className="mb-4 text-sm text-stone-500">
            ...
            {listings.length === 1 ? "woning gevonden" : "woningen gevonden"}
        ...
        {listings.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-stone-200 bg-white px-6 py-16 text-center shadow-sm">
            ...
            <p className="mt-2 max-w-sm text-sm leading-relaxed text-stone-500">
              ...
              momenteel nog geen woningen die exact overeenkomen.
            </p>
